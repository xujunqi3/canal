...
public class RdbConfigMonitor {
    ...
    private class FileListener extends FileAlterationListenerAdaptor {
        ...
        public void onFileChange(File file) {
            ...
            try {
                if (phoenixAdapter.getRdbMapping().containsKey(file.getName())) {
                    // 加载配置文件
        ...
        private void addConfigToCache(File file, MappingConfig mappingConfig) {
            ...
            phoenixAdapter.getRdbMapping().put(file.getName(), mappingConfig);
            if (!mappingConfig.getDbMapping().getMirrorDb()) {
                ...
                Map<String, MirrorDbConfig> mirrorDbConfigCache = phoenixAdapter.getMirrorDbConfigCache();
        ...
        private void deleteConfigFromCache(File file) {
            MappingConfig mappingConfig = phoenixAdapter.getRdbMapping().remove(file.getName());
            ...
            if (!mappingConfig.getDbMapping().getMirrorDb()) {
                ...
                phoenixAdapter.getMirrorDbConfigCache().forEach((key, mirrorDbConfig) -> {
                    if (mirrorDbConfig.getFileName().equals(file.getName())) {
                        phoenixAdapter.getMirrorDbConfigCache().remove(key);
                    }
                });
            }
